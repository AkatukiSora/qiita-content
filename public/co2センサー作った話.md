...
Raspberry Pi Imagerで焼き込むときにWiFiの設定(有線の場合は不要)、ユーザーの設定、SSH公開鍵を焼き込んでおくと、最初からSSH接続できるので楽です。

InfluxDBはラズパイからネットワーク経由でアクセスできる場所へデプロイします。
GrafanaはInfluxDBにアクセスできる場所へデプロイしましょう。
我が家では外出中でもアクセスできるようKubernetesへデプロイし、CloudFlare Tunnel経由で公開しています。
...
デプロイできたことを確認したら公式ドキュメントに従って初期設定します。
...
必要に応じてリソース制限、StorageClass、サービスを変更します。
...
pythonはなんかvenvというものを使わないと使えなくなったみたいなので先に設定しておいてください。
このあとはvenv環境で動かしている前提で書いていきます。

```bash:bash
pythom -m pip install mh_z19
```

pythonコマンド経由でpipを呼び出すのはvenvのpipを使うためです。
pipでグローバルにパッケージをインストールできないようになっているようなので。
...
私の環境だとvenvを有効化したうえで`venv/bin/python3 -m mh_z19`としないと呼び出せませんでした。<!-- textlint-disable-->
なにか間違えたかもしれません。
<!-- textlint-enable-->
...
コード最後の方はCO2データの読み出しに結構時間がかかっていたので、厳密に5秒間隔でデータを取るようちょっとした細工しています。
...
目的のデータだけが見えるようになったら`SCRIPT EDITOR`を押してfluxクエリを表示します。
それをすべてコピーしてGrafanaに移動します。

先にGrafanaでinfluxDBの情報が見えるようにデータソースとして登録します。
ここはおそらく案内があるのでその通りに作成してください。
ここでもapiキーはすべてのバケットに対してreadする権限だけにしています。
最小権限の原則ってやつです(2回目)。

データソースとして登録できたらダッシュボードとパネルを作成します。
パネルの設定画面でデータソースをinfluxDBへ設定し、queryの欄へ先ほどコピーしたものを貼り付けます。
